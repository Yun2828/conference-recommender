...
from utils import kw_input_UI, nl_input_UI, gs_input_UI, configure
...
if 'prompt_items' not in st.session_state:
     st.session_state.prompt_items = {'Google Scholar': "",
                                      "Keyword": "",
                                      "Natural Language": ""}
...
input_type1 = st.sidebar.segmented_control(label = "Select input type",
                          options = ["Key Term",
                                     "Natural Language",
                                     "Use My Google Scholar Profile"],
...
if filter_special_sessions:
    ...
    if input_type == "Keyword":
        ...
        nl_input_UI()
    elif input_type == "Use My Google Scholar Profile":
        gs_input_UI()
        ...
        if (input_type == "Keyword"):
            try:
                if st.session_state.prompt_items["Keyword"][0] != '':
                    process_modified(input_type = input_type, output_pres = output_pres, display_schedule = display_schedule)
            except:
                pass
        elif (input_type == "Natural Language" and st.session_state.prompt_items["Natural Language"][0] != ''):
            process_modified(input_type = input_type, output_pres = output_pres, display_schedule = display_schedule)
        elif (input_type == "Use My Google Scholar Profile" and 
            isinstance(st.session_state.prompt_items["Google Scholar"],pd.DataFrame)):
            try:
                process_modified(input_type = input_type, output_pres = output_pres, display_schedule = display_schedule)
            except Exception:
                pass
    ...
    if st.session_state.search_triggered:
        if (input_type == "Keyword"):
            try:
                if st.session_state.prompt_items["Keyword"][0] != '':
                    process(input_type = input_type, output_pres = output_pres, display_schedule = display_schedule)
            except:
                pass
        elif (input_type == "Natural Language" and st.session_state.prompt_items["Natural Language"][0] != ''):
            process(input_type = input_type, output_pres = output_pres, display_schedule = display_schedule)
